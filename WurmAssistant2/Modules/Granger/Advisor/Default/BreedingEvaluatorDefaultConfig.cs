...
namespace Aldurcraft.WurmOnline.WurmAssistant2.ModuleNS.Granger
{
    ...
    {
        private DefaultBreedingEvaluator.Options OptionsRef;
        ...
        public BreedingEvaluatorDefaultConfig(DefaultBreedingEvaluator.Options options)
        {
            OptionsRef = options;
            InitializeComponent();
            ...
            numericUpDownBadTraitWeight.Value = GeneralHelper.ConstrainValue<decimal>((decimal)OptionsRef.BadTraitWeight, 0, 100);
            ...
            numericUpDownPotValGoodWeight.Value = GeneralHelper.ConstrainValue<decimal>((decimal)OptionsRef.PotentialValuePositiveWeight, 0, 100);
            numericUpDownPotValBadWeight.Value = GeneralHelper.ConstrainValue<decimal>((decimal)OptionsRef.PotentialValueNegativeWeight, 0, 100);

            checkBoxPreferUniqueTraits.Checked = OptionsRef.PreferUniqueTraits;
            numericUpDownUniqueTraitWeight.Value = GeneralHelper.ConstrainValue<decimal>((decimal)OptionsRef.UniqueTraitWeight, 0, 100);

            numericUpDownInbreedPenaltyWeight.Value = GeneralHelper.ConstrainValue<decimal>((decimal)OptionsRef.InbreedingPenaltyWeight, 0, 100);
            ...
            olvColumnColorWeight.AspectPutter +=
                ((rowObject, value) =>
                {
                    var colorWeight = (DefaultBreedingEvaluator.ColorWeight)rowObject;
                    try
                    {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        Logger.LogInfo("Attempted to insert invalid value to color weight", this, exception);
                    }
                });
        ...
        {
            toolTip1.SetToolTip(ctrl, tip.GetLineWrappedStringEx());
        }
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            OptionsRef.HorseColorValues =
                ((IEnumerable<DefaultBreedingEvaluator.ColorWeight>)objectListViewColorWeights.Objects).ToArray();
        }
        ...
        private void objectListViewColorWeights_FormatCell(object sender, BrightIdeasSoftware.FormatCellEventArgs e)
        {
            ...
            {
                var cweight = (DefaultBreedingEvaluator.ColorWeight)e.Model;
                var color = cweight.Color.ToSystemDrawingColor();
                if (color != null)
                {
                    e.SubItem.BackColor = color.Value;
                    e.SubItem.ForeColor = e.SubItem.BackColor.GetContrastingWhiteOrBlackColorEx();
                }
            }
        }
