...
namespace Aldurcraft.Utility
{
    ...
    public static class Logger
    {
        ...
        const int MAX_ALLOWED_MEGABYTES_FOR_LOGS = 32;
        ...
        /// </summary>
        public static string LogSavePath { get; set; }

        static string BuildLogSavePathForThisSession()
        {
            if (LogSaveDir == null) return null;
            _logSavePath = Path.Combine(LogSaveDir, DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss-fff") + ".txt");
            return _logSavePath;
        }
        ...
        static void TryCleanUpOldLogs()
        {
            try
            {
                var cleaner = new LoggerOldLogsCleaner(LogSaveDir);

                long totalBytes = cleaner.GetByteCount();
                TimeSpan ts = TimeSpan.FromDays(62);

                while (totalBytes > 1024 * 1024 * MAX_ALLOWED_MEGABYTES_FOR_LOGS)
                {
                    ts -= TimeSpan.FromDays(14);
                    totalBytes -= cleaner.DoCleanup(DateTime.Now - ts);
                    if (ts < TimeSpan.FromDays(7)) break;
                }
            }
            catch (Exception _e)
            {
        ...
        static IoExtensions.StringWriterFlushEvent ConsoleOutStringWriter = null;
        ...
        public static void SetConsoleHandlingMode(ConsoleHandlingOption option)
        {
            try
            {
                    ...
                    || option == ConsoleHandlingOption.SendConsoleToLoggerOutputINFO)
                {
                    ...
                    {
                        ConsoleOutStringWriter = new IoExtensions.StringWriterFlushEvent(true);
        ...
        static void ConsoleOutStringWriter_Flushed(object sender, IoExtensions.StringWriterFlushEventArgs e)
        {
        ...
        public static bool SetLogSaveDir(string directoryPath)
        {
            ...
            LogSavePath = BuildLogSavePathForThisSession();
            LogInfo("Text File Logging started at " + DateTime.Now.ToString());
